...
import com.first.saccelerator.model.AppInfoPassVPNResponse;
...
public class AppInfoPassVPNDataSource extends BaseDataSource<AppInfoPassVPNResponse.BaseBean> {
    ...
    public AppInfoPassVPNDataSource(DBHelper dbHelper) {
        super(dbHelper);
    }

    @Override
    public long insert(AppInfoPassVPNResponse.BaseBean baseBean) {
        return insert(baseBean.getName(), baseBean.getPackageName(), baseBean.getPackagePath(), baseBean.getVersionName(),
                baseBean.getVersionCode(), baseBean.getIsSystem(), baseBean.getIsAllowed());
    }
...
//        values.put(Colum.ICON, bitmaptoByte(brawabletoBitmap(icon)));
    ...
    @Override
    public boolean insertList(List<AppInfoPassVPNResponse.BaseBean> list) {
        result = false;
        // 批量插入开启事务能提高效率
        mDb.beginTransaction();
        try {
            for (AppInfoPassVPNResponse.BaseBean bean : list) {
                insertOrUpdate(bean);
            }
    ...
    @Override
    public long insertOrUpdate(AppInfoPassVPNResponse.BaseBean baseBean) {
        return insertOrUpdate(baseBean.getName(), baseBean.getPackageName(), baseBean.getPackagePath(), baseBean.getVersionName(),
                baseBean.getVersionCode(), baseBean.getIsSystem(), baseBean.getIsAllowed());
    }
    ...
    @Override
    public int update(AppInfoPassVPNResponse.BaseBean baseBean) {
        return update(baseBean.getName(), baseBean.getPackageName(), baseBean.getPackagePath(), baseBean.getVersionName(),
                baseBean.getVersionCode(), baseBean.getIsSystem(), baseBean.getIsAllowed());
    }
...
//        values.put(Colum.ICON, bitmaptoByte(brawabletoBitmap(icon)));
    ...
    @Override
    public AppInfoPassVPNResponse.BaseBean find(String findId) {
        return null;
    }

    @Override
    public List<AppInfoPassVPNResponse.BaseBean> findAll() {
        List<AppInfoPassVPNResponse.BaseBean> list = new ArrayList<>();
        Cursor cursor = null;
        try {
            ...
            if (cursor != null) {
                while (cursor.moveToNext()) {
                    ...
                    list.add(new AppInfoPassVPNResponse.BaseBean(primaryid, name, packagename, packagepath, versionname, versioncode, issystem, isallowed));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * 查找允许通过VPN的应用数据
     */
    public List<AppInfoPassVPNResponse.BaseBean> findVPNAllow() {
        List<AppInfoPassVPNResponse.BaseBean> list = new ArrayList<>();
        Cursor cursor = null;
        try {
            cursor = mDb.query(TABLE_NAME, new String[]{
     ...
     * Drawable 转 Bitmap
     */
    public Bitmap brawabletoBitmap(Drawable drawable) {
